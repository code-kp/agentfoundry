...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Create a new workspace agent scaffold.")
    parser.add_argument(
        "--workspace-root",
    ...
    parser.add_argument(
        "--workspace-package",
        default="workspace",
        help="Import package name for the workspace.",
    )
    args = parser.parse_args(argv)

    workspace_root = Path(args.workspace_root).resolve()
    run_agent_scaffold_wizard(
        workspace_root,
        workspace_package=args.workspace_package,
    )
    return 0
